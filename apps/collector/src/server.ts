...
export function createApi(service: TrackerService, token: string, logger: Logger) {
 return http.createServer(async (req, res) => {
  ...
  try {
   if (!/^127\.0\.0\.1:\d+$/.test(req.headers.host ?? '')) return send(403, { error: 'HOST_REJECTED' });
   const origin = req.headers.origin;
   if (origin && !/^chrome-extension:\/\/[a-p]{32}$/.test(origin)) return send(403, { error: 'ORIGIN_REJECTED' });
   ...
   if (req.method === 'GET' && url.pathname === '/status') return send(200, service.status(url.searchParams.get('clientId') ?? ''));
   ...
   for await (const chunk of req) { const bytes = Buffer.from(chunk as Uint8Array); size += bytes.length; if (size > 4 * 1024 * 1024) { send(413, { error: 'PAYLOAD_TOO_LARGE' }); req.destroy(); return; } chunks.push(bytes); }
   const input: unknown = JSON.parse(Buffer.concat(chunks).toString('utf8'));
   if (url.pathname === '/events') return send(200, service.ingest(sampleSchema.parse(input)));
   if (url.pathname === '/control') {
    const action = z.object({ action: z.enum(['pause', 'resume', 'ignore', 'finish', 'sync', 'shutdown']), clientId: z.string().max(200).default('') }).parse(input);
    if (action.action === 'shutdown') { await service.shutdown(); send(200, { ok: true }); process.emit('SIGTERM'); return; }
   ...
   if (!res.headersSent) send(bad ? 400 : 500, { error: bad ? 'INVALID_REQUEST' : 'INTERNAL_ERROR' });
  }
 });
}
