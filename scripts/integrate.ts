...
function update(file: string, transform: (old: string) => string) {
  const old = files.read(file);
  if (old === null) throw new Error("INTEGRATION_TARGET_MISSING: " + file);
  if (files.write(file, transform(old), old)) changes.push(file);
}
update("首页/学习热力图.md", (old) => {
  if (old.includes("// GPT_TRACKER_HEATMAP_SOURCE")) return old;
  const needle = "const today=new Date();";
  if (!old.includes(needle)) throw new Error("HEATMAP_LAYOUT_CHANGED");
  return old.replace(
    needle,
    `// GPT_TRACKER_HEATMAP_SOURCE\nfor (const p of dv.pages('"${daily}"')) {\n  const d = String(p.date ?? '').match(/\\d{4}-\\d{2}-\\d{2}/)?.[0];\n  if (!d) continue;\n  const previous = byDate.get(d);\n  byDate.set(d, {total: (previous?.total ?? 0) + (Number(p.ai_minutes) || 0), path: previous?.path ?? p.file.path});\n}\n${needle}`,
  );
});
...
const guidePath = c.obsidian.dataFolder + "/使用说明.md",
  guideOld = files.read(guidePath);
if (guideOld === null) {
  files.write(guidePath, guide, null);
  changes.push(guidePath);
}
const statsPath = c.obsidian.dataFolder + "/daily-stats.json";
if (files.read(statsPath) === null) {
  files.write(statsPath, "{}\n", null);
  changes.push(statsPath);
}
fs.mkdirSync(path.join(PROJECT_ROOT, "work"), { recursive: true });
fs.writeFileSync(
  path.join(PROJECT_ROOT, "work/integration-result.json"),
  JSON.stringify(
    {
      at: new Date().toISOString(),
      changes,
      records: store.db
        .prepare("SELECT * FROM sync_records ORDER BY created_at")
        .all(),
    },
    null,
    2,
