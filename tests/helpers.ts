...
export function setup() {
  const dir = path.join(root, "work", "tests", randomUUID());
  fs.mkdirSync(path.join(dir, "vault/.obsidian"), { recursive: true });
  const config = JSON.parse(
    fs.readFileSync(path.join(root, "config/default.json"), "utf8"),
  ) as Config;
  config.vaultPath = path.join(dir, "vault");
  const store = new Store(
    path.join(dir, "data/test.sqlite3"),
    path.join(root, "apps/collector/migrations"),
  );
